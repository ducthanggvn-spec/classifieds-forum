export function parseBBCode(text: string) {
  ...
  let html = text
    ...
    // Hỗ trợ thẻ quote có tên người trích dẫn: [quote=nickname]nội dung[/quote]
    .replace(/\[quote=(.*?)\]([\s\S]*?)\[\/quote\]/gi, "<blockquote class='border-l-4 border-blue-500 bg-blue-50 dark:bg-blue-900/30 p-3 my-3 rounded text-sm italic'><div class='font-bold text-blue-700 dark:text-blue-400 mb-1'>$1 viết:</div>$2</blockquote>")
    // Hỗ trợ thẻ quote thường: [quote]nội dung[/quote]
    .replace(/\[quote\]([\s\S]*?)\[\/quote\]/gi, "<blockquote class='border-l-4 border-gray-400 bg-gray-50 dark:bg-gray-800 p-3 my-3 rounded text-sm italic'>$1</blockquote>")
